function [batchResult, pipelineResult] = resultParser(fn)
    ...
    batchTree = tree.getElementsByTagName('batch');
    pipelineTree = tree.getElementsByTagName('pipeline');
    
    function result = parseResult(tree)
    
        inputs = tree.item(0).getElementsByTagName('input');
    ...
    batchResult = parseResult(batchTree);
    pipelineResult = parseResult(pipelineTree);
end